...
#print sys.argv[2]
try:
	i = os.system("/usr/local/bin/sipp "+ sys.argv[1]+" -i "+sys.argv[2]+ " -p 5095 -s +14027658834 -sf /etc/sipp/proxytesting/OC15-uac.xml -rtp_echo -m "+sys.argv[3]+ " >/etc/sipp/proxytesting/OC15.txt")    

#Filtering out the contents of .txt file and confirming if the test case is Pass/Fail

	f = open("/etc/sipp/proxytesting/OC15.txt","r")
	y = f.readlines()
